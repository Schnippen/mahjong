...
function isPotentialComplete(
  tile: TTileObject,
  discardedTile: TTileObject,
  visibleTileCounts: tileCountsType,
): boolean {
  ...
  if (type && value) {
    ...
    if (
      visibleTileCounts[nextValue1] > 0 &&
      visibleTileCounts[nextValue2] > 0
    ) {
      return true;
    }
  }
